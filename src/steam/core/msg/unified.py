...
def get_um(method_name, response=False):
    ...
    if key not in method_lookup:
        ...
        for method_desc in service.GetDescriptor().methods:
            ...
            method_lookup[(name, False)] = getattr(
                package, method_desc.input_type.full_name, None
            )
            method_lookup[(name, True)] = getattr(
                package, method_desc.output_type.full_name, None
            )
