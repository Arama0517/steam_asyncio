...
class Util_Functions(unittest.TestCase):
    ...
    def test_ip6_to_bytes(self):
        ...
        self.assertEqual(
            ut.ip6_to_bytes('1:2:3:4:5:6:7:8'),
            b'\x00\x01\x00\x02\x00\x03\x00\x04\x00\x05\x00\x06\x00\x07\x00\x08',
        )
        self.assertEqual(
            ut.ip6_to_bytes('1234:5678:9abc:def0:0:dead:beef:1'),
...
class Util_Proto(unittest.TestCase):
    ...
    def test_proto_from_dict_to_dict(self):
        DATA = {
            'buffers': [
                {
                    'data': b'some data',
                    'flags': [{'flag': True}, {'flag': False}, {'flag': False}],
                },
                {
                    'data': b'\x01\x02\x03\x04',
                    'flags': [{'flag': False}, {'flag': True}, {'flag': True}],
                },
            ],
            'list_number32': [
                4,
    ...
    def test_proto_fill_from_dict__dict_generator(self):
        utp.proto_fill_from_dict(
            self.msg, {'messages': (x for x in [{'text': 'one'}, {'text': 'two'}])}
        )
        self.assertEqual(len(self.msg.messages), 2)
        self.assertEqual(self.msg.messages[0].text, 'one')
        self.assertEqual(self.msg.messages[1].text, 'two')
    ...
    def test_proto_fill_from_dict__dict_map(self):
        utp.proto_fill_from_dict(
            self.msg, {'messages': map(dict, [{'text': 'one'}, {'text': 'two'}])}
        )
        self.assertEqual(len(self.msg.messages), 2)
        self.assertEqual(self.msg.messages[0].text, 'one')
        self.assertEqual(self.msg.messages[1].text, 'two')
    ...
    def test_proto_fill_from_dict__dict_filter(self):
        utp.proto_fill_from_dict(
            self.msg,
            {'messages': filter(lambda x: True, [{'text': 'one'}, {'text': 'two'}])},
        )
        self.assertEqual(len(self.msg.messages), 2)
        self.assertEqual(self.msg.messages[0].text, 'one')
        self.assertEqual(self.msg.messages[1].text, 'two')
