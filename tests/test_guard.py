...
class TCguard(unittest.TestCase):
    def test_generate_twofactor_code_for_time(self):
        code = g.generate_twofactor_code_for_time(
            b'superdupersecret', timestamp=3000030
        )
        self.assertEqual(code, 'YRGQJ')

        code = g.generate_twofactor_code_for_time(
            b'superdupersecret', timestamp=3000029
        )
    ...
    def test_generate_confirmation_key(self):
        ...
        self.assertEqual(
            key, b'\xed\xb5\xe5\xad\x8f\xf1\x99\x01\xc8-w\xd6\xb5 p\xccz\xd7\xd1\x05'
        )
